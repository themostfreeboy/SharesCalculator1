...
void CSharesCalculatorDlg::OnBnClickedBcalculate()
{
	...
	{
		if(f_Num_Buy_1>0)		f_result_YJ+=(f_Price_Buy_1*f_Num_Buy_1*f_YJ*0.001<5)?5:(f_Price_Buy_1*f_Num_Buy_1*f_YJ*0.001);
		if(f_Num_Buy_2>0)		f_result_YJ+=(f_Price_Buy_2*f_Num_Buy_2*f_YJ*0.001<5)?5:(f_Price_Buy_2*f_Num_Buy_2*f_YJ*0.001);
		if(f_Num_Buy_3>0)		f_result_YJ+=(f_Price_Buy_3*f_Num_Buy_3*f_YJ*0.001<5)?5:(f_Price_Buy_3*f_Num_Buy_3*f_YJ*0.001);
		if(f_Num_Buy_4>0)		f_result_YJ+=(f_Price_Buy_4*f_Num_Buy_4*f_YJ*0.001<5)?5:(f_Price_Buy_4*f_Num_Buy_4*f_YJ*0.001);
		if(f_Num_Buy_5>0)		f_result_YJ+=(f_Price_Buy_5*f_Num_Buy_5*f_YJ*0.001<5)?5:(f_Price_Buy_5*f_Num_Buy_5*f_YJ*0.001);
		if(f_Num_Sell_1>0)		f_result_YJ+=(f_Price_Sell_1*f_Num_Sell_1*f_YJ*0.001<5)?5:(f_Price_Sell_1*f_Num_Sell_1*f_YJ*0.001);
		if(f_Num_Sell_2>0)		f_result_YJ+=(f_Price_Sell_2*f_Num_Sell_2*f_YJ*0.001<5)?5:(f_Price_Sell_2*f_Num_Sell_2*f_YJ*0.001);
		if(f_Num_Sell_3>0)		f_result_YJ+=(f_Price_Sell_3*f_Num_Sell_3*f_YJ*0.001<5)?5:(f_Price_Sell_3*f_Num_Sell_3*f_YJ*0.001);
		if(f_Num_Sell_4>0)		f_result_YJ+=(f_Price_Sell_4*f_Num_Sell_4*f_YJ*0.001<5)?5:(f_Price_Sell_4*f_Num_Sell_4*f_YJ*0.001);
		if(f_Num_Sell_5>0)		f_result_YJ+=(f_Price_Sell_5*f_Num_Sell_5*f_YJ*0.001<5)?5:(f_Price_Sell_5*f_Num_Sell_5*f_YJ*0.001);
	}
	//计算异地通讯费
	...
	if(f_result_PRICE*f_result_NUM>=5)//券商交易佣金验证
	{
	...
	{
		f_result_PRICE=(f_temp_GHF+5-f_result_in_PROFITS)/(1-f_YHS*0.001-f_TXF*0.001-f_Self_Sell*0.001-f_Self_BuySell*0.001)/f_result_NUM;
		if(f_result_PRICE*f_result_NUM<5)//券商交易佣金验证
		{
			cstr_temp.Format(_T("%.2f"),f_result_PRICE);
	        SetDlgItemText(IDC_ERESULT_PRICE,cstr_temp);//设置剩余股票不亏本最低卖出价格结果
...
{
	// TODO: 在此添加控件通知处理程序代码
	AfxMessageBox(_T("过户费收费标准：每1000股收取1元，不足1000股按1元收取。仅上海股票收取。买入卖出时均收取。"),MB_OK,NULL);
}
...
{
	// TODO: 在此添加控件通知处理程序代码
	AfxMessageBox(_T("券商交易佣金收费标准：成交金额的1‰-3‰(上限为3‰)，具体根据证券公司决定，但是最低收取标准是5元。深圳股票上海股票均收取。买入卖出时均收取。"),MB_OK,NULL);
}
...
void CSharesCalculatorDlg::OnBnClickedCdisableGhf()
{
	...
	else if(DisableGHF==false)//非禁用
	{
		if(ShareType==0)             SetDlgItemText(IDC_EGHF,_T("0"));//深圳股票(深)设置过户费为0‰
		    else if(ShareType==1)    SetDlgItemText(IDC_EGHF,_T("1"));//上海股票(沪)设置过户费为1‰
		CEdit *edit = (CEdit*)GetDlgItem(IDC_EGHF);
...
void CSharesCalculatorDlg::OnBnClickedCdisableYj()
{
	...
	{
		SetDlgItemText(IDC_EYJ,_T("3"));//设置券商交易佣金为3‰
		CEdit *edit = (CEdit*)GetDlgItem(IDC_EYJ);
